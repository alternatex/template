...
TODOS
-----
- marry/examples w/store !!!
- think naming convention for templates -> add conversion table firstname => customerFirstname -> nested value prevail on last node select and therefore might lead to unwanted output due to the approach working with a single-level path atm -> TODO: enable both (debug out node driven path as a helper at least)
- add magic function to allow derivedby/computedby/xxx?
- special classes -> optional -> don't process node if no data avail?! or just hide -> parametrizeee
- accessor pointer?! -> store wrap also resetting it --> x --> depth -> y <- depth -- a -- x --> depth -> y
- complete weld functionality support » compat yay/nay ?!
- wrapper for weld? » loadash underscore compat alike version
- how to handle node removal -> check @ all calls » update object references if not avail (temp array store avail) => create new accessor function with these..
- allow classes w > ./attributename ../attribute name 
- » store path crumb .. = ref to parent at child before changing the pointer ref!!
- scope all *
- fix function compilation
- improve code generation => location helpers instead of using data-attribute on element nodes/cleanup?!
- think - compiled code on for arrays?! » refactor compilation function generation => inject accessors -> always compile * 
- move README documentation part to wiki and just ref it from readme -> add wiki as submodule -> folder: /docs ... 
- performance monitor -> check how lodash does it's checks against underscore
...
// helper to process array contents
...
var assign = function assign(node, pointer, className){
  if ( node.nodeType == 3 ) {
    ...
    if(node.nodeName=="INPUT" && (node.type=="text" || node.type=="email" || node.type=="password" || node.type=="hidden"))
...
// TODO: property in accessor containing datastructure path * -> stringify flat? possible?

// ...
var callall = function callall(data, databinder, attribute, value, compile, name){
...
TODOS
-----
- for all array elements compile -> can also be nested -> adjust examples
- handle object references => contacts => friends => wich are contacts... => silng.include alike solve => include(context, data)
...
var wrapwalker2 = function wrapwalker2(fragment, data, type, databinder, compile, name){
  ...
  if(typeof(data.uuid)=='undefined') {
    ...
    data.all = function all(attribute, value){ callall(data, databinder, attribute, value); };
  }
